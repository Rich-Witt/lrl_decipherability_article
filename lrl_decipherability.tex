...
\usepackage{hyperref}
\hypersetup{
  pdftitle={Informational Content Analysis of Undeciphered Scripts},
...
\begin{abstract}
%% Text of abstract
Deciphering lost scripts creates new sources of evidence to understand how language evolved, verify archaeological evidence, and rediscover lost knowledge. Low resource languages, particularly, undeciphered scripts, present a particular challenge for Natural Language Processing since the small corpus size requires us to determine if the available informational content is sufficient for decipherment to be theoretically possible. This paper outlines several methods used to aid in decipherment. Unicity distance is calculated for Rongorongo, Linear A, and the Indus Valley Script to determine decipherability. These three lost scripts are then analyzed to determine the type of writing system using Zipf's Law and HDBSCAN. By combining these two analyses it is found that the Linear A corpus does have enough informational content for decipherment regardless of assumed writing type, however the Rongorongo and Indus Valley corpora only have enough information if we assume they are logographic. [I will be able to make some statement like this, but likely not as strong.]
...
\section{Data}
\label{data}
Two distinct data sets were used. One for Rongo Rongo and one for Linear A.
Linear A
Indus valley script is
Rongo Rongo is the extinct script of Easter Island 


\subsection{Rongo Rongo}
Both the analysis of Rongo Rongo with variants and  Rongo Rongo analysis without variants was conducted using data available at the \href{http://kohaumotu.org/Rongorongo/Glyph_Library/glyph_variants.html}{Rongorongo corpus (codes only)}.This data is \cite{}.
Initially recorded by Barthel in 1958 \cite{}.  
Impressive interactive website greatly aiding in the understanding of Rongo Rongo glyphs.
We make no assertion on the accuracy 
26 Tablets Coded A though Z. 
from \href{http://kohaumotu.org/Rongorongo_new/views/codes_only.php}{the Kohaumotu Rongorongo codes page}.

Create a corpus of all encodings for Rongorongo based on updated 3 character
glyph codes.

Insead of using the glyphs themeselves we rely directly on the index created by Barthel as it represents a manual encoding of glyphs.

Glyphs are numbered in a systematic manner.  
Each row is intended to categorize the gliphs with Row 0 containing the simplest forms while higher rows increase in complexity.  
While only 90 rows are shown 799 total rows were categorized by Barthel and avaliable in \cite{} and in the Kohaumotu.org cite. 

To create the index used in this analysis the row and column are concatenated to result in the three diget index used.
